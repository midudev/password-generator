...
const CardKevin = () => {
  ...
  return (
    <div className="min-h-screen p-2   flex flex-col justify-center items-center font-sans ">
      <h2 className=" text-lg mb-5 text-[#61606c] text-center">
        Password Generator
      </h2>
          ...
          <Level
            nivel={
              Object.entries(Options).filter((d) => d[1] === true).length <= 1
                ? 'LOW'
                : Object.entries(Options).filter((d) => d[1] === true)
                    .length === 4
                ? 'HIGH'
                : 'MEDIUM'
            }
            quantity={
              Object.entries(Options).filter((d) => d[1] === true).length
            }
          ></Level>
        </div>
        <button
          onClick={createPassword}
          className="w-full mt-5 text-[#18171f] bg-[#a3ffae] py-3 "
        >
          GENERATE +
        </button>
      </div>
