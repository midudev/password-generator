...
const Circle = ({ bgColor, children }) => (
	<div className={styles.circle} style={{ backgroundColor: bgColor }}>
		{children}
	</div>
)

const ConsoleLine = ({ children, path }) => (
	<section className={styles.console_line}>
		<span className={styles.command_user}>soujiro17@hacktoberfest-2022: </span>
		<span className={styles.simbol}>{path || '～'}</span>$ {children}
	</section>
)
...
const PasswordGenerator = () => {
	...
	useEffect(() => {
		inputPasswordLength.current[refIndex].selectionStart =
			inputPasswordLength.current[refIndex].value.length
		inputPasswordLength.current[refIndex].selectionEnd =
			inputPasswordLength.current[refIndex].value.length
	}, [passwordLengthList.length])
	...
	return (
		<div className={styles.generate_password_container}>
			...
			<div className={styles.generate_password_content}>
				...
				<ConsoleLine path='/mnt/c'>
					py generate_password.py --length{' '}
					<input
						ref={(ref) => (inputPasswordLength.current[0] = ref)}
						value={passwordLengthList[0] || passwordLength}
						onChange={handlePasswordLength}
						onKeyDown={handleKeyPress}
						className={styles.command_input}
						disabled={passwordLengthList.length > 1}
					/>
				</ConsoleLine>
				{password.map((pass, i) => (
					<React.Fragment key={pass}>
						...
						<ConsoleLine path='/mnt/c'>
							py generate_password.py --length{' '}
							<input
								ref={(ref) => (inputPasswordLength.current[i] = ref)}
								value={passwordLengthList[i + 1] || passwordLength}
								onChange={handlePasswordLength}
								onKeyDown={handleKeyPress}
								className={styles.command_input}
							/>
