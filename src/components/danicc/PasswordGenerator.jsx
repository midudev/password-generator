		...
		<div className="flex flex-col gap-y-2 text-slate-200">
			...
			<Card>
				<form className='flex flex-col gap-y-4' onSubmit={handleGenerate}>
					...
					<input className="my-4 accent-yellow-300" id="character-length" type="range" min="1" max="30" name="characterLength" value={rangeValue} onChange={({ target: { value } }) => { setRangeValue(value) }} />


					{Object.entries(checkboxOptions).map(([optionId, optionLabel]) => (
						<label htmlFor={optionId} key={optionId}>
							<input type="checkbox" id={optionId} name={optionId} value={optionId} className='mr-2 accent-yellow-300' />
							{optionLabel}
						</label>
					))}


					<button type="submit" className="border border-yellow-300 py-2 text-yellow-300">Generate</button>
				</form>
			</Card>
		</div>
	)
