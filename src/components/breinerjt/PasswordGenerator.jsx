...
export const PasswordGenerator = () => {
	...
	return (
		<div className='grid gap-4 place-self-center min-w-[304px] p-4 rounded-lg bg-gradient-to-t from-gray-700 via-gray-400 to-gray-600'>
			<section className={`relative flex items-center bg-slate-800 rounded-md ${isCopied ? 'border border-[#b4ff39]' : 'border border-transparent'}`}>
				<button
					aria-label='Copy url button'
					className={`p-2 pr-1 bg-slate-600 rounded-tl-md rounded-bl-md ${isPasswordEmpty ? 'opacity-20' : ''}`}
			...
			<section className='text-center'>
				<Slider
					max={maxValue}
					min={minValue}
					setPasswordLength={setPasswordState}
				/>
				<button
