...
import HacktoberfestImg from './components/HacktoberfestImg'
import usePasswordConfig from './hooks/usePasswordConfig'
import useCopy from './hooks/useCopy'
import useTranslation from './hooks/useTranslation'

export default function App() {
	// [!] `Prop drilling` isn't good, but this is a little app, dont worry ;)

	const { t, lang, toggleLang } = useTranslation()
	const { onClick, Copied } = useCopy(t)
	const {
		setLength,
		length,
		PREFERENCES,
		...
		<Container>
			...
			<Container.Main>
				...
				<Button
					...
					disabled={IS_WRONG_PASSWORD}
				/>

				<ErrorText
					isVisible={!IS_SECURE_PASSWORD && !IS_WRONG_PASSWORD}
					text={t('insecurePassword')}
					style={{ transform: 'translateY(1.2rem)' }}
				/>

				<ErrorText
					isVisible={IS_WRONG_PASSWORD}
				...
				<Preferences title={t('preferences')}>
					{PREFERENCES.map((props) => (
						<Preferences.Item {...props} key={props.text} />
					))}

					<Preferences.Range
						desc={`${t('length')} (${length})`}
						value={length}
